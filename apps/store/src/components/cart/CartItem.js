...
export default function CartItem({
  ...
  const getItemImage = () => {
    ...
    if (item.product_data?.primary_image) return item.product_data.primary_image;
    if (item.product_snapshot?.primary_image) return item.product_snapshot.primary_image;
  ...
  const showExpandButton = hasExtraDetails();
    ...
    <div className="p-3 md:p-6 relative">
      <div className="flex gap-2 md:gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            {isUpdating && <span className="text-xs md:text-sm text-gray-500">Updating...</span>}
          </div>
        </div>
      </div>
      ...
      {isExpanded && showExpandButton && (
        <div className="mt-4 pt-4 border-t border-gray-200 space-y-3">
          {/* Variant Details */}
          {item.variant && (
            <div className="rounded-lg p-3" style={{ backgroundColor: `${primaryColor}05` }}>
              <h4 className="text-sm font-semibold text-gray-900 mb-2">Variant Details</h4>
              <div className="grid grid-cols-2 gap-2 text-sm">
                {item.variant.color && (
                  <div>
                    <span className="text-gray-500">Color:</span>
                    <span className="ml-2 font-medium text-gray-900">{item.variant.color}</span>
                  </div>
                )}
                {item.variant.size && (
                  <div>
                    <span className="text-gray-500">Size:</span>
                    <span className="ml-2 font-medium text-gray-900">{item.variant.size}</span>
                  </div>
                )}
                {item.variant.sku && (
                  <div className="col-span-2">
                    <span className="text-gray-500">Variant SKU:</span>
                    <span className="ml-2 font-mono text-xs text-gray-900">{item.variant.sku}</span>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="space-y-1 text-sm">
                {item.product_snapshot.brand && (
                  <div>
                    ...
                    <span className="ml-2 font-medium text-gray-900">{item.product_snapshot.brand}</span>
                  </div>
                )}
                {item.product_snapshot.unit_of_measure && (
                  <div>
                    <span className="text-gray-500">Unit:</span>
                    <span className="ml-2 font-medium text-gray-900">{item.product_snapshot.unit_of_measure}</span>
                  </div>
                )}
              </div>
            </div>
