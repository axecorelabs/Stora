...
export default function OrderDetailsContent({
  ...
  // Payment-level check only (shared with the Orders list's own quick-
  // action Refund button -- see apps/dashboard/src/lib/orderRefund.js),
  // not the full canRefundOrder() (which also needs refundSplit, absent
  // when this component is fed the Orders-list popup's shape). No need to
  // wait on a background fetch just to decide whether to show the
  // button: RefundModal fetches the canonical order itself once opened,
  // and the API's own atomic guard correctly rejects an already-reversed
  // split as a normal form error, same as any other rejected submission.
  const canRefund = () => mightBeRefundable(order);
