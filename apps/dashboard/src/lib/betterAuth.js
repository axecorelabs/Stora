...
// (email/password's own recordLegalAcceptance() call, and the
// review-and-accept interstitial after Google) are each responsible for
// clearing this back to NULL. A future signup path nobody wires that
// into fails closed (stays gated) instead of silently exempt.
async function flagPendingLegalReview(userId) {
  await pool.query(
    `UPDATE users SET legal_review_pending_at = NOW() WHERE id = $1`,
    [userId]
  );
}
      ...
      create: {
        after: async (user) => {
          ...
          try {
            ...
            console.error('Failed to flag pending legal review for new user:', error);
          }
        }
      }
    },
