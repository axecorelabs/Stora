-- First-class business category at the store/listing level.
--
-- Item categories remain unchanged and still power product/service-level
-- filtering. This adds store-level classification for business discovery,
-- AI query routing, and future dashboard tailoring.
...
-- Keep values predictable while still allowing NULL during rollout/backfill.
ALTER TABLE stores
  DROP CONSTRAINT IF EXISTS stores_business_category_check;

ALTER TABLE stores
  ADD CONSTRAINT stores_business_category_check
  CHECK (
    business_category IS NULL
    OR business_category IN ('retail', 'restaurant', 'services', 'hybrid', 'other')
  );

CREATE INDEX IF NOT EXISTS idx_stores_business_category ON stores(business_category);

-- Backfill obvious existing stores so search can use this immediately.
UPDATE stores
SET business_category = CASE
  WHEN restaurant_mode = true THEN 'restaurant'
  WHEN offers_services = true AND (sells_products = true OR restaurant_mode = true) THEN 'hybrid'
  WHEN offers_services = true THEN 'services'
  WHEN sells_products = true THEN 'retail'
  ELSE 'other'
END
WHERE business_category IS NULL;

-- search_vendors / search_vendors_ai gain p_business_category so vendor
-- discovery can use first-class business type filters. Drop prior signatures
-- first to avoid accidental overload ambiguity.
DROP FUNCTION IF EXISTS public.search_vendors(text, text, integer, integer, text[], text, text, boolean, text);
DROP FUNCTION IF EXISTS public.search_vendors_ai(vector, text[], text, text, boolean, integer, integer, text);
